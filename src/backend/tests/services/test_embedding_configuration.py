...
def _delete_builtin_embedding_providers(db: Session) -> None:
    for provider in db.exec(
        select(models.EmbeddingProvider).where(
...
def test_default_model_embedding_is_disabled_without_builtin_provider(db: Session):
    _delete_builtin_embedding_providers(db)
...
def test_seed_builtin_embedding_provider_uses_split_gateway_models(monkeypatch):
    monkeypatch.setattr(
        settings,
    ...
    monkeypatch.setattr(settings, "BUILTIN_EMBEDDING_PROVIDER_NAME", "builtin-embedding-test", raising=False)
    monkeypatch.setattr(settings, "EMBEDDING_MODEL", "fallback-embedding", raising=False)
    monkeypatch.setattr(settings, "EMBEDDING_TEXT_MODEL", "text-embedding", raising=False)
    monkeypatch.setattr(settings, "EMBEDDING_CODE_MODEL", "code-embedding", raising=False)

    provider = init_db.seed_builtin_embedding_provider()

    assert provider is not None
    assert provider.user_id is None
    assert provider.is_builtin is True
    assert provider.visible_to_all is True
    assert provider.type == EmbeddingProviderType.LOCAL
    assert provider.mode == EmbeddingMode.SPLIT
    assert provider.text_base_url == "http://gateway:9090/litellm_proxy/${TEAM_ID}/{accessToken}/v1"
    assert provider.code_base_url == "http://gateway:9090/litellm_proxy/${TEAM_ID}/{accessToken}/v1"
    assert provider.text_api_key == "EMPTY"
    assert provider.code_api_key == "EMPTY"
    assert provider.text_model == "text-embedding"
    assert provider.code_model == "code-embedding"


def test_default_model_selects_builtin_embedding_provider(db: Session, monkeypatch):
    monkeypatch.setattr(
        settings,
        "BUILTIN_PROVIDER_BASE_URL",
        "http://gateway:9090/litellm_proxy/${TEAM_ID}/{accessToken}/v1",
    ...
    monkeypatch.setattr(settings, "BUILTIN_EMBEDDING_PROVIDER_NAME", "builtin-embedding-default-test", raising=False)
    monkeypatch.setattr(settings, "EMBEDDING_MODEL", "fallback-embedding", raising=False)
    monkeypatch.setattr(settings, "EMBEDDING_TEXT_MODEL", "", raising=False)
    monkeypatch.setattr(settings, "EMBEDDING_CODE_MODEL", "", raising=False)
    monkeypatch.setattr(
    ...
    assert defaults.embedding_provider_id == provider.id
    assert defaults.embedding_provider_name == provider.name
