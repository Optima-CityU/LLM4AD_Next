...
from app.schemas.paper import PaperRuntimeSessionCreate, PaperRuntimeSessionResponse
from app.services import credential_broker, knowledge_service, paper_service, paper_workflow
...
async def create_runtime_session(
    ...
    missing_prerequisites = [
        ...
        if ((workspace.proposal_stage_states or {}).get(stage) or {}).get("status") != "ready"
    ]
    ...
    profile = {
        ...
        "env": {
            ...
            "ANTHROPIC_BASE_URL": settings.LLM_PROXY_BASE_URL.rstrip("/"),
        ...
        "systemPromptAppend": (
            ...
            "Use the installed stage Skill as the method and stay within the current project workspace."
        ),
    }
...
async def publish_runtime_stage_result(
    ...
    publication_key = f"paper_runtime_publication:{request.run_id}:{request.idempotency_key}"
    redis = await get_async_redis()
