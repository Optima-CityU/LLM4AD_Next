...
def _load_registry() -> tuple[dict[str, Any], ...]:
    ...
    registry（``topics.yaml``）只是**索引**：``topic`` / ``metric_key`` /
    ``metric_direction`` 三个字段是早期手写简报，与 manifest 大面积漂移（详见
    :func:`list_topics`），下游一律以 manifest 为准。
...
def _manifest_map(available_ids: set[str] | None = None) -> dict[str, dict[str, Any]]:
    """一次性把注册表里所有课题的 manifest 读成 ``{id: manifest}``。

    registry 只用来取 id 列表，其余全部来自 manifest。读不到的（manifest 缺失 /
    YAML 坏）按空 dict 处理——picker 仍要列出该课题，让它报错不如让它降级显示。

    先按 :func:`topic_exists` 之外的路径直接读文件：``load_manifest`` 每调一次都会
    先线性扫一遍注册表，55 个课题就是 55×55 次比较，这里只需要 id 集合已在手，
    故走 ``prepare_run.load_manifest`` 本体。

    Args:
        available_ids: 只读这些 id（``None`` = 注册表内全部）。``list_topics`` 带
            ``domain`` 过滤时传进来，避免为被过滤掉的域白读一遍 manifest。
    """
    out: dict[str, dict[str, Any]] = {}
    if available_ids is None:
        available_ids = {str(t.get("id")) for t in _load_registry() if t.get("id")}
    for tid in available_ids:
        try:
            out[tid] = dict(_prepare_run().load_manifest(tid))
        except HTTPException:
            logger.debug(f"manifest unreadable for topic {tid}", exc_info=True)
            out[tid] = {}
    return out


# manifest 里 ``metrics[].direction`` 是自由文本（除 maximize/minimize 外还有
# match_reference / characterize / target_0.95 等语义值，共 14 种）。只有
# maximize/minimize 能进 ARC 的 ``experiment.metric_direction``，语义值里含义明确的
# 归一化，拿不准的留空（空串 = 未指定，ARC 自己判定；猜错方向会让 Stage-13 反向择优）。
_SEMANTIC_MINIMIZE: frozenset[str] = frozenset(
    {
        "match",
        "match_reference",
        "minimize_abs",
        "minimize_given_coverage",
        "characterize",
        "estimate",
        "report",
        "diagnostic",
...
def _primary_metric(manifest: dict[str, Any]) -> tuple[str, str]:
    """取 manifest 的首个 metric，返回 ``(name, direction)``。

    与 ``create_session``（``sessions.py``）和 ``run_bench_init.materialize_config``
    同款取法：都认 ``metrics[0]`` 是主指标。

    Returns:
        ``(metric_key, metric_direction)``；缺失时为 ``("", "")``。
    """
    ...
    raw = str(primary.get("direction") or "").strip().lower()
    if raw in ("maximize", "minimize"):
        direction = raw
    elif raw in _SEMANTIC_MINIMIZE:
        direction = "minimize"
    else:
        direction = ""
...
def list_topics(domain: str | None = None) -> list[dict[str, Any]]:
    ...
    字段全部取自 **manifest**（课题的真源）。registry（``topics.yaml``）只贡献
    id 列表和 ``domains`` 学科标签——它的 ``topic`` / ``metric_key`` /
    ``metric_direction`` 是早期手写简报，全量对账下 55 个课题里 23 个
    ``metric_key`` 与 manifest 的 ``metrics[0].name`` 不一致（P/B/S 三个域 20 个
    课题一律写占位符 ``primary_metric``）、15 个 ``metric_direction`` 不一致。
    前端 CreateSessionDialog 会把这里的值回填成表单默认值并提交，后端
    ``create_session`` 的回落是 ``if not metric_key``——回填过的值会被当成"用户
    显式指定"而跳过 manifest，于是错误的列名一路进 ARC 的 ``experiment.metric_key``，
    Stage-13 择优按它取不到值。
    ...
    selected = [t for t in registry if not domain or t.get("domain") == domain]
    manifests = _manifest_map(
        {str(t.get("id")) for t in selected if t.get("id")}
    )
    items: list[dict[str, Any]] = []
    for topic in selected:
        tid = str(topic.get("id") or "")
        sub = topic.get("domain") or ""
        manifest = manifests.get(tid) or {}
        metric_key, metric_direction = _primary_metric(manifest)
        items.append(
            {
                "id": tid,
                "title": _topic_title(topic, manifest),
                # 副标题与会话题面同一个公式（derive_topic），picker 里看到的
                # 就是建会话后 ARC 收到的题面。
                "topic": derive_topic(manifest) if manifest else (topic.get("topic") or ""),
...
def _topic_title(topic: dict[str, Any], manifest: dict[str, Any]) -> str:
    """课题展示名：manifest 的 ``title``（干净、完整，最长 160 字符）。

    registry 的 ``topic`` 字段是完整题面（较长，picker 里当副标题展示），只作兜底。
    不按句号切分——``E. coli …`` / ``arXiv:hep-ph/9909255`` 这类缩写会被切出
    单字符标题。manifest 读不到（空 dict）时退回截断到 80 字符的题面。
    """
    tid = topic.get("id")
...
def get_template_detail(topic_id: str) -> dict[str, Any]:
    ...
    # list_topics() 已按 manifest 取值；这里补上 manifest 全文供预览。
    summary = next(
